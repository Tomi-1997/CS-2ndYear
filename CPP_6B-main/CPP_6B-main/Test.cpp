...
TEST_CASE("Test Schedule")
{
    ...
    CHECK_NOTHROW(League l);
    League l;
    CHECK_NOTHROW(Schedule s(&l));
    bool valid_wins = false;

    for (int i = 0; i < 20; i++)
    {
        CHECK((l.getTeam(i)->getAwayWins() >= 0 && l.getTeam(i)->getHomeWins() <= 20) == true);
        if (l.getTeam(i)->getWins() > 10)
        {
            valid_wins = true;
        }
    }
